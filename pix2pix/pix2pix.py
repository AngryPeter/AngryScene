...
parser.add_argument("--test_path", type=str, default="../dataset/val_A")
parser.add_argument("--output_path", type=str, default="./result_ndiff")
...
# 在数据集内部定义 transform 确保 image 和 label 同时翻转
dataloader = ImageDataset(opt.train_path, mode="train").set_attrs(
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.n_cpu,
)
val_dataloader = ImageDataset(opt.test_path, mode="test").set_attrs(
    batch_size=10,
    shuffle=False,
    num_workers=1,
)
...
def eval(epoch, writer):
    ...
    for i, (_, real_A, photo_id) in enumerate(val_dataloader):
        ...
        for idx in range(fake_B.shape[0]):
            cv2.imwrite(f"{opt.output_path}/images/test_fake_imgs/epoch_{epoch}/{photo_id[idx]}.jpg",
                        fake_B[idx].transpose(1, 2, 0)[:, :, ::-1])
            cnt += 1

if opt.epoch != 0:
    # Load pretrained models
    generator.load(f"./generator_{opt.epoch}.pkl")
    discriminator.load(f"./discriminator_{opt.epoch}.pkl")
    eval(100, writer)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    # for i, (real_B, real_A, _) in enumerate(dataloader):
        ...
        fake_B = generator(real_A)
        # fake_B = diff_augment(fake_B, policy=policy)
        # real_B = diff_augment(real_B, policy=policy)
