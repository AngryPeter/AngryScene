...
class UIModel(BaseModel):
    ...
    def toTensor(self, img, normalize=False):
        tensor = jt.float32(np.array(img, np.int32, copy=False))
        tensor = jt.reshape(tensor, (1, img.size[1], img.size[0], len(img.mode)))
        tensor = jt.transpose(tensor, (1, 2))
        tensor = jt.transpose(tensor, (1, 3))
        # tensor.transpose((1, 2)).transpose((1, 3)).contiguous()
        if normalize:
    ...
    def load_image(self, label_path, inst_path, feat_path):
        ...
        input_label = jt.zeros([1, opt.label_nc, h, w])
        # input_label = self.Tensor(torch.Size(oneHot_size)).zero_()
        self.input_label = jt.misc.scatter_(input_label, 1, jt.int64(label_map), 1.0)
        ...
        if not opt.no_instance:
            ...
            self.net_input = jt.concat((self.input_label, self.edge_map), dim=1)
        else:
        ...
        object_np = self.object_map.numpy().astype(int)
        self.feat_map = jt.zeros([1, opt.feat_num, h, w])
        ...
        for i in np.unique(object_np):
            ...
            if label in self.features_clustered:
                ...
                idx = jt.misc.nonzero(self.object_map == i)
    ...
    def get_edges(self, t):
        edge = jt.uint8(jt.zeros(t.size()))
        edge[:, :, :, 1:] = edge[:, :, :, 1:] | (t[:, :, :, 1:] != t[:, :, :, :-1])
        edge[:, :, :, :-1] = edge[:, :, :, :-1] | (t[:, :, :, 1:] != t[:, :, :, :-1])
        edge[:, :, 1:, :] = edge[:, :, 1:, :] | (t[:, :, 1:, :] != t[:, :, :-1, :])
        edge[:, :, :-1, :] = edge[:, :, :-1, :] | (t[:, :, 1:, :] != t[:, :, :-1, :])
        return jt.float32(edge)
    ...
    def change_labels(self, click_src, click_tgt):
        ...
        idx_src = jt.misc.nonzero(self.inst_map == inst_src)
        ...
        if idx_src.shape:
            ...
            idx_tgt = jt.misc.nonzero(self.inst_map == inst_tgt)
            if idx_tgt.shape:
    ...
    def add_strokes(self, click_src, label_tgt, bw, save):
        ...
        h, w = size[2], size[3]
        idx_src = jt.int64(jt.zeros([bw ** 2, 4]))
        for i in range(bw):
    ...
    def add_objects(self, click_src, label_tgt, mask, style_id=0):
        ...
        idx_src = jt.misc.nonzero(jt.float32(mask))
    ...
    def single_forward(self, net_input, feat_map):
        net_input = jt.concat((net_input, feat_map), dim=1)
        fake_image = self.netG.execute(net_input)
    ...
    def update_features(self, cluster_idx, mask=None, click_pt=None):
        ...
        if mask is not None:
            ...
            mask = np.transpose(mask, (2, 0, 1))[np.newaxis, ...]
            idx = jt.misc.nonzero(jt.float32(mask))
            # idx = torch.from_numpy(mask).cuda().nonzero()
            idx[:, 2] += y
            idx[:, 3] += x
            # changing the label of an existing object
        else:
            idx = jt.misc.nonzero(self.object_map == self.instToChange)
    ...
    def get_current_visuals(self, getLabel=False):
        ...
        if self.mask is not None:
            mask = np.transpose(self.mask[0].float().numpy(), (1, 2, 0)).astype(np.uint8)
